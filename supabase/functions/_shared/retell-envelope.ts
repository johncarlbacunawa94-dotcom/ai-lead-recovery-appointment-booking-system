...
export function parseRetellToolEnvelope(
  ...
  if (!isRecord(parsed.call)) {
    return {
      ok: false,
      errorCode: "INVALID_REQUEST",
      message: "Retell call context is required.",
    };
  }
  ...
  if (!isNonEmptyString(parsed.call.call_id)) {
    return {
      ok: false,
      errorCode: "INVALID_REQUEST",
      message: "Retell call_id is required.",
    };
  }

  if (
    parsed.call.call_type !== "web_call" &&
    parsed.call.call_type !== "phone_call"
  ) {
    return {
      ok: false,
      errorCode: "INVALID_REQUEST",
      message: "Retell call_type is invalid.",
    };
  }
  ...
  if (!isRecord(parsed.args)) {
    return {
      ok: false,
      errorCode: "INVALID_REQUEST",
      message: "Function args must be an object.",
    };
  }

  const call: RetellCallContext = {
    call_id: parsed.call.call_id,
    call_type: parsed.call.call_type,
  };

  if (isNonEmptyString(parsed.call.call_status)) {
    call.call_status = parsed.call.call_status;
  }

  if (isNonEmptyString(parsed.call.agent_id)) {
    call.agent_id = parsed.call.agent_id;
  }

  if (
    typeof parsed.call.agent_version === "number" &&
    Number.isInteger(parsed.call.agent_version)
  ) {
    call.agent_version =
  ...
  if (
    parsed.call.direction === "inbound" ||
    parsed.call.direction === "outbound"
  ) {
    call.direction = parsed.call.direction;
  }
